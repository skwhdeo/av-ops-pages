import{B as we,l as m,d as Ne,c as ie,H as Ce,a as o,t as e,x as E,C as f,G as C,z as he,F as b,f as M,I as H,v as _,D as ne,u as Se,O as ge,o as r}from"./vendor-CJYFYo_D.js";import{l as ve,_ as Re}from"./index-BIiQ34-I.js";import{b as Oe}from"./menus-api-CvNMhl-t.js";import{b as Le}from"./menu-api-endpoints-api-A4i8MDTV.js";const Ue=we("menuApiEndpoints",()=>{const T=m([]),c=m(!1),d=m(null),I=new Le(ve()),G=async i=>{try{c.value=!0,d.value=null,console.log("📡 Loading menu API endpoints for menu:",i);const u=await I.getMenuApiEndpointsApiMenusMenuIdApiEndpointsGet(i);u.data?(T.value=Array.isArray(u.data)?u.data:[u.data],console.log("✅ Loaded menu API endpoints:",T.value)):T.value=[]}catch(u){console.error("❌ Failed to load menu API endpoints:",u),d.value=`메뉴 API 엔드포인트 조회 실패: ${u.message}`,T.value=[]}finally{c.value=!1}};return{endpoints:T,loading:c,error:d,loadMenuApiEndpoints:G,searchMenuApiEndpoints:async(i,u,n)=>{try{c.value=!0,d.value=null,console.log("📡 Searching menu API endpoints:",{path:i,method:u,menuId:n});const h=await I.searchMenuApiEndpointsApiMenusApiEndpointsSearchGet(i,u,n);if(h.data){const w=Array.isArray(h.data)?h.data:[h.data];return console.log("✅ Found menu API endpoints:",w),w}return[]}catch(h){return console.error("❌ Failed to search menu API endpoints:",h),d.value=`메뉴 API 엔드포인트 검색 실패: ${h.message}`,[]}finally{c.value=!1}},createMenuApiEndpoint:async(i,u)=>{try{c.value=!0,d.value=null,console.log("📡 Creating menu API endpoint:",i,u);const n=await I.createMenuApiEndpointApiMenusMenuIdApiEndpointsPost(i,u);return console.log("✅ Created menu API endpoint:",n.data),await G(i),n.data}catch(n){throw console.error("❌ Failed to create menu API endpoint:",n),d.value=`메뉴 API 엔드포인트 생성 실패: ${n.message}`,n}finally{c.value=!1}},updateMenuApiEndpoint:async(i,u,n)=>{try{c.value=!0,d.value=null,console.log("📡 Updating menu API endpoint:",i,u,n);const h=await I.updateMenuApiEndpointApiMenusMenuIdApiEndpointsEndpointIdPut(i,u,n);return console.log("✅ Updated menu API endpoint:",h.data),await G(i),h.data}catch(h){throw console.error("❌ Failed to update menu API endpoint:",h),d.value=`메뉴 API 엔드포인트 수정 실패: ${h.message}`,h}finally{c.value=!1}},deleteMenuApiEndpoint:async(i,u)=>{try{c.value=!0,d.value=null,console.log("📡 Deleting menu API endpoint:",i,u),await I.deleteMenuApiEndpointApiMenusMenuIdApiEndpointsEndpointIdDelete(i,u),console.log("✅ Deleted menu API endpoint"),await G(i)}catch(n){throw console.error("❌ Failed to delete menu API endpoint:",n),d.value=`메뉴 API 엔드포인트 삭제 실패: ${n.message}`,n}finally{c.value=!1}},httpMethods:["GET","POST","PUT","DELETE","PATCH"],getMethodColor:i=>({GET:"#10b981",POST:"#3b82f6",PUT:"#f59e0b",DELETE:"#ef4444",PATCH:"#8b5cf6"})[i]||"#6b7280",hasWildcard:i=>i.includes("*")}}),oe=[{path:"/api/auth/token",method:"POST",summary:"토큰 발급",description:"OAuth2 토큰 발급 (password / client_credentials grant)",operationId:"login_for_access_token_api_auth_token_post",tags:["auth"]},{path:"/api/auth/login",method:"POST",summary:"사용자 로그인 (별칭)",description:"토큰 엔드포인트의 별칭 - 일반적인 REST API 패턴 지원",operationId:"login_alias_api_auth_login_post",tags:["auth"]},{path:"/api/auth/logout",method:"POST",summary:"로그아웃",description:"로그아웃 이벤트를 기록합니다",operationId:"logout_api_auth_logout_post",tags:["auth"]},{path:"/api/auth/my-permissions",method:"GET",summary:"현재 사용자의 메뉴 권한 조회",description:"현재 로그인한 사용자의 메뉴별 CRUD 권한을 조회합니다 (프론트엔드 버튼 제어용)",operationId:"get_my_permissions_api_auth_my_permissions_get",tags:["auth"]},{path:"/api/auth/2fa/enable",method:"POST",summary:"Enable 2Fa",description:`2FA 활성화 시작 - QR 코드 및 백업 코드 생성
...
- 차량 상태 업데이트, WebSocket 브로드캐스트, ETA 계산 등 실시간 처리는 하지 않음`,operationId:"save_pvd_batch_api_pvd_batch_post",tags:["pvd"]},{path:"/api/pvd/message",method:"POST",summary:"PVD 통합 메시지 수신 (JWT HS256/ES256 또는 vehicle_id)",description:`PVD 메시지 통합 수신 엔드포인트

세 가지 인증 방식 지원:

1. JWT Bearer 토큰 - HS256 (서버 발급, 운전자 앱):
   \`\`\`bash
   # 운전자가 /api/auth/token으로 로그인하여 받은 토큰 사용
   curl -X POST http://localhost:8000/api/pvd/message          -H "Authorization: Bearer <server_token>"          -H "Content-Type: application/json"          -d '{"latitude": 35.1, "longitude": 129.0}'
   \`\`\`

2. JWT Bearer 토큰 - ES256 (차량 자체 서명):
   \`\`\`bash
   # 차량의 private key로 서명한 JWT 사용
   curl -X POST http://localhost:8000/api/pvd/message          -H "Authorization: Bearer <vehicle_jwt>"          -H "Content-Type: application/json"          -d '{"latitude": 35.1, "longitude": 129.0}'
   \`\`\`

3. vehicle_id 파라미터 (레거시, 인증 없음):
   \`\`\`bash
   curl -X POST "http://localhost:8000/api/pvd/message?vehicle_id=1"          -H "Content-Type: application/json"          -d '{"latitude": 35.1, "longitude": 129.0}'
   \`\`\`

JWT 방식이 보안상 권장됩니다.

응답:
- auth_method: "jwt_server" (HS256) / "jwt_vehicle" (ES256) / "vehicle_id" (레거시)`,operationId:"receive_pvd_message_api_pvd_message_post",tags:["pvd"]},{path:"/api/webhooks/register",method:"POST",summary:"Register Webhook",description:`Webhook 등록
...
Returns:
    현재 연결 상태 정보`,operationId:"get_websocket_status_api_websocket_status_get",tags:["websocket"]},{path:"/api/health",method:"GET",summary:"Health Check",description:"시스템 상태 및 DB 연결 풀 상태 확인",operationId:"health_check_api_health_get",tags:["monitoring"]},{path:"/",method:"GET",summary:"프론트엔드 리다이렉트",description:"루트 경로 접속 시 정적 HTML 페이지로 리다이렉트",operationId:"read_root__get",tags:["system"]},{path:"/api/version",method:"GET",summary:"시스템 버전 정보",description:"현재 배포된 시스템의 Git 커밋 정보 조회",operationId:"get_version_api_version_get",tags:["system"]}];function Ve(){return oe}function Fe(T){if(!T)return oe;const c=T.toLowerCase();return oe.filter(d=>d.path.toLowerCase().includes(c)||d.method.toLowerCase().includes(c)||d.summary.toLowerCase().includes(c)||d.description.toLowerCase().includes(c)||d.operationId.toLowerCase().includes(c)||d.tags.some(I=>I.toLowerCase().includes(c)))}const He={class:"menu-management"},We={class:"filters-section"},$e={class:"filter-group"},xe={key:0,class:"menu-tree"},Be={class:"tree-container"},je={class:"node-content"},Je={class:"node-info"},Qe=["onClick"],Ke={key:1,class:"expand-icon",style:{visibility:"hidden"}},ze={class:"menu-icon"},qe={class:"menu-name"},Xe={class:"menu-path"},Ye={class:"menu-order"},Ze={key:2,class:"api-count-badge"},et={class:"node-actions"},tt=["onClick"],at=["onClick"],st=["onClick"],it=["onClick"],nt=["onClick"],ot={key:0,class:"tree-children"},rt={class:"node-content"},pt={class:"node-info"},_t=["onClick"],dt={key:1,class:"expand-icon",style:{visibility:"hidden"}},lt={class:"menu-icon"},ct={class:"menu-name"},mt={class:"menu-path"},ut={class:"menu-order"},ht={key:2,class:"api-count-badge"},gt={class:"node-actions"},vt=["onClick"],yt=["onClick"],It=["onClick"],Et=["onClick"],Tt=["onClick"],kt={key:0,class:"tree-children"},At={class:"node-content"},Dt={class:"node-info"},ft={class:"menu-icon"},bt={class:"menu-name"},Mt={class:"menu-path"},Gt={class:"menu-order"},Pt={key:0,class:"api-count-badge"},wt={class:"node-actions"},Nt=["onClick"],Ct=["onClick"],St=["onClick"],Rt=["onClick"],Ot={key:1,class:"loading-container"},Lt={key:2,class:"empty-state"},Ut={class:"modal-content"},Vt={class:"modal-header"},Ft={class:"modal-body"},Ht={class:"form-group"},Wt={class:"form-group"},$t={class:"form-group"},xt={class:"form-group"},Bt=["value"],jt={class:"form-group"},Jt={class:"form-actions"},Qt=["disabled"],Kt={class:"modal-content modal-wide"},zt={class:"modal-header"},qt={class:"modal-body"},Xt={class:"permission-table-container"},Yt={class:"permission-table"},Zt={class:"role-name"},ea={class:"permission-cell"},ta=["checked","onChange"],aa={class:"permission-cell"},sa=["checked","onChange"],ia={class:"permission-cell"},na=["checked","onChange"],oa={class:"permission-cell"},ra=["checked","onChange"],pa={class:"permission-cell"},_a=["checked","onChange"],da={class:"form-actions"},la=["disabled"],ca={class:"modal-content modal-large"},ma={class:"modal-header"},ua={class:"modal-body"},ha={class:"endpoint-form"},ga={class:"endpoint-filters"},va={class:"form-group"},ya={class:"form-group"},Ia=["value"],Ea={class:"form-group"},Ta=["value"],ka={class:"available-endpoints-list"},Aa={class:"available-endpoints-count"},Da={class:"available-endpoints-scroll"},fa=["onClick"],ba={class:"endpoint-detail"},Ma={class:"endpoint-path"},Ga={class:"endpoint-summary"},Pa=["disabled"],wa={class:"endpoints-list"},Na={key:0,class:"empty-message"},Ca={key:1,class:"endpoint-items"},Sa={class:"endpoint-info"},Ra={class:"endpoint-path"},Oa={class:"endpoint-desc"},La=["onClick"],Ua=Ne({__name:"MenuManagementView",setup(T){const c=new Oe(ve()),d=Ue(),I=m([]),G=m([]),S=m(!1),k=m(!1),q=m(null),R=m(""),A=m(new Set),X=m(),O=m(!1),i=m({name:"",path:"",icon:"",parent_id:null,sort_order:999}),u=m(!1),n=m(null),h=m(new Map),w=m(!1),P=m([]),Y=m(!1),W=m([]),$=m([]),L=m(""),D=m(null),U=m(""),V=m(""),F=m(new Map),re=["ADMIN","MANAGER","OPERATOR","DRIVER","VIEWER"],ye=["GET","POST","PUT","DELETE","PATCH"],Ie=ie(()=>{const s=new Set;return W.value.forEach(t=>{t.tags.forEach(p=>s.add(p))}),Array.from(s).sort()}),pe=ie(()=>{const s=I.value.filter(g=>{var a;if(R.value){const v=R.value.toLowerCase();if(!g.name.toLowerCase().includes(v)&&!((a=g.path)!=null&&a.toLowerCase().includes(v)))return!1}return!0}),t=s.filter(g=>!g.parent_id),p=g=>s.filter(a=>a.parent_id===g).map(a=>({...a,children:p(a.id)})).sort((a,v)=>(a.sort_order||999)-(v.sort_order||999));return t.map(g=>({...g,children:p(g.id)})).sort((g,a)=>(g.sort_order||999)-(a.sort_order||999))}),Ee=ie(()=>I.value.filter(s=>{let t=0,p=s;for(;p.parent_id&&t<2;)p=I.value.find(g=>g.id===p.parent_id)||p,t++;return t<2}).map(s=>{const t=s.parent_id?I.value.find(p=>p.id===s.parent_id):null;return{...s,parent_name:t==null?void 0:t.name}})),x=async()=>{try{S.value=!0,q.value=null;const s=await c.getMenusApiMenusGet();if(s.data){const t=Array.isArray(s.data)?s.data:s.data.items||[];I.value=t,t.filter(p=>!p.parent_id).forEach(p=>A.value.add(p.id))}await _e()}catch(s){console.error("Failed to load menus:",s),q.value=s.message}finally{S.value=!1}},_e=async()=>{try{const s=[];for(const t of re)try{const p=await c.getRoleMenuPermissionsApiMenusPermissionsRoleGet(t);if(p.data&&p.data.menus){const g=p.data.menus.filter(a=>a.id!==void 0).map(a=>({role:p.data.role,menu_id:a.id,can_read:a.can_read??!1,can_create:a.can_create??!1,can_update:a.can_update??!1,can_delete:a.can_delete??!1}));s.push(...g)}}catch{}s.length>0&&(G.value=s)}catch(s){console.error("Failed to load permissions:",s)}},B=s=>{s?i.value={id:s.id,name:s.name,path:s.path,icon:s.icon,parent_id:s.parent_id,sort_order:s.sort_order}:i.value={name:"",path:"",icon:"",parent_id:null,sort_order:999},O.value=!0},de=s=>{i.value={name:"",path:s.path+"/",icon:"",parent_id:s.id,sort_order:999},O.value=!0},j=()=>{O.value=!1,i.value={}},Te=async()=>{try{k.value=!0,i.value.id?await c.updateMenuApiMenusMenuIdPut(i.value.id,i.value):await c.createMenuApiMenusPost(i.value),j(),await x()}catch(s){console.error("Failed to save menu:",s),alert("메뉴 저장 실패: "+s.message)}finally{k.value=!1}},Z=async s=>{if(confirm(`"${s.name}" 메뉴를 삭제하시겠습니까?`))try{await c.deleteMenuApiMenusMenuIdDelete(s.id),await x()}catch(t){console.error("Failed to delete menu:",t),alert("메뉴 삭제 실패: "+t.message)}},le=s=>{A.value.has(s)?A.value.delete(s):A.value.add(s)},ee=s=>{n.value=s,h.value.clear(),G.value.filter(t=>t.menu_id===s.id&&t.role).forEach(t=>{t.role&&h.value.set(t.role,{can_read:t.can_read||!1,can_create:t.can_create||!1,can_update:t.can_update||!1,can_delete:t.can_delete||!1})}),u.value=!0},J=()=>{u.value=!1,n.value=null,h.value.clear()},Q=(s,t)=>{const p=h.value.get(s);return p?p[t]:!1},K=(s,t)=>{const p=h.value.get(s)||{can_read:!1,can_create:!1,can_update:!1,can_delete:!1};p[t]=!p[t],h.value.set(s,{...p})},ce=s=>{const t=h.value.get(s);return t?t.can_read&&t.can_create&&t.can_update&&t.can_delete:!1},ke=s=>{const t=ce(s);h.value.set(s,{can_read:!t,can_create:!t,can_update:!t,can_delete:!t})},Ae=async()=>{var s;if(n.value)try{k.value=!0;for(const[t,p]of h.value.entries()){const g=p.can_read||p.can_create||p.can_update||p.can_delete;try{let v=(await c.getRoleMenuPermissionsApiMenusPermissionsRoleGet(t)).data.menu_ids||[];g?v.includes(n.value.id)||v.push(n.value.id):v=v.filter(l=>l!==n.value.id),v.length>0?await c.updateRoleMenuPermissionsApiMenusPermissionsRolePut(t,{role:t,menu_ids:v}):await c.deleteRoleMenuPermissionsApiMenusPermissionsRoleDelete(t)}catch(a){if(((s=a.response)==null?void 0:s.status)===404)g&&await c.setRoleMenuPermissionsApiMenusPermissionsPost({role:t,menu_ids:[n.value.id]});else throw a}}await _e(),J()}catch(t){console.error("Failed to save permissions:",t),alert("권한 저장 실패: "+t.message)}finally{k.value=!1}},De=s=>({ADMIN:"관리자",MANAGER:"매니저",OPERATOR:"운영자",DRIVER:"운전자",VIEWER:"조회자"})[s]||s,fe=()=>{clearTimeout(X.value),X.value=setTimeout(()=>{},300)},te=async s=>{n.value=s,w.value=!0,await d.loadMenuApiEndpoints(s.id),P.value=d.endpoints,W.value=Ve(),L.value="",U.value="",V.value="",D.value=null,z()},ae=()=>{w.value=!1,n.value=null,P.value=[],W.value=[],$.value=[]},z=()=>{let s=W.value;L.value&&(s=Fe(L.value)),U.value&&(s=s.filter(t=>t.method===U.value)),V.value&&(s=s.filter(t=>t.tags.includes(V.value))),$.value=s},be=s=>{D.value=s},Me=async()=>{if(!(!n.value||!D.value))try{Y.value=!0,await d.createMenuApiEndpoint(n.value.id,{menu_id:n.value.id,method:D.value.method,path:D.value.path,description:D.value.summary||void 0}),await d.loadMenuApiEndpoints(n.value.id),P.value=d.endpoints,F.value.set(n.value.id,d.endpoints.length),D.value=null}catch(s){console.error("Failed to add API endpoint:",s),alert("API 엔드포인트 추가 실패: "+s.message)}finally{Y.value=!1}},Ge=async s=>{if(n.value&&confirm("이 API 엔드포인트를 삭제하시겠습니까?"))try{await d.deleteMenuApiEndpoint(n.value.id,s),await d.loadMenuApiEndpoints(n.value.id),P.value=d.endpoints,F.value.set(n.value.id,d.endpoints.length)}catch(t){console.error("Failed to delete API endpoint:",t),alert("API 엔드포인트 삭제 실패: "+t.message)}},me=s=>d.getMethodColor(s),N=s=>F.value.get(s)||0,Pe=async()=>{try{for(const s of I.value)try{await d.loadMenuApiEndpoints(s.id),F.value.set(s.id,d.endpoints.length)}catch{F.value.set(s.id,0)}}catch(s){console.error("Failed to load menu API counts:",s)}};return Ce(async()=>{await x(),await Pe()}),(s,t)=>{var p,g;return r(),o("div",He,[t[29]||(t[29]=e("div",{class:"page-header"},[e("h1",null,"메뉴 관리"),e("p",{class:"subtitle"},"시스템 메뉴와 권한을 관리합니다.")],-1)),e("div",We,[e("div",$e,[t[10]||(t[10]=e("label",null,"검색",-1)),f(e("input",{"onUpdate:modelValue":t[0]||(t[0]=a=>R.value=a),type:"text",class:"form-control",placeholder:"메뉴명, 경로 검색",onInput:fe},null,544),[[C,R.value]])]),e("button",{onClick:t[1]||(t[1]=a=>B()),class:"btn btn-primary"},t[11]||(t[11]=[e("span",{class:"icon"},"+",-1),he(" 새 메뉴 ",-1)])),e("button",{onClick:x,class:"btn btn-secondary"},t[12]||(t[12]=[e("span",{class:"icon"},"🔄",-1),he(" 새로고침 ",-1)]))]),S.value?(r(),o("div",Ot,t[14]||(t[14]=[e("div",{class:"spinner"},null,-1),e("p",null,"메뉴를 불러오는 중...",-1)]))):(r(),o("div",xe,[e("div",Be,[(r(!0),o(b,null,M(pe.value,a=>{var v;return r(),o("div",{key:a.id,class:"tree-node"},[e("div",je,[e("div",Je,[a.children&&a.children.length?(r(),o("span",{key:0,class:"expand-icon",onClick:l=>le(a.id)},_(A.value.has(a.id)?"▼":"▶"),9,Qe)):(r(),o("span",Ke,"▶")),e("span",ze,_(a.icon||"📄"),1),e("span",qe,_(a.name),1),e("span",Xe,_(a.path),1),e("span",Ye,"["+_(a.sort_order)+"]",1),N(a.id)>0?(r(),o("span",Ze," API: "+_(N(a.id)),1)):E("",!0)]),e("div",et,[e("button",{onClick:l=>te(a),class:"btn-icon",title:"API 엔드포인트"}," 📡 ",8,tt),e("button",{onClick:l=>ee(a),class:"btn-icon",title:"권한 설정"}," 🔐 ",8,at),e("button",{onClick:l=>B(a),class:"btn-icon",title:"수정"},"✏️",8,st),e("button",{onClick:l=>de(a),class:"btn-icon",title:"하위 메뉴 추가"}," ➕ ",8,it),(v=a.children)!=null&&v.length?E("",!0):(r(),o("button",{key:0,onClick:l=>Z(a),class:"btn-icon",title:"삭제"}," 🗑️ ",8,nt))])]),a.children&&a.children.length&&A.value.has(a.id)?(r(),o("div",ot,[(r(!0),o(b,null,M(a.children,l=>{var ue;return r(),o("div",{key:l.id,class:"tree-node child-node"},[e("div",rt,[e("div",pt,[l.children&&l.children.length?(r(),o("span",{key:0,class:"expand-icon",onClick:y=>le(l.id)},_(A.value.has(l.id)?"▼":"▶"),9,_t)):(r(),o("span",dt,"▶")),e("span",lt,_(l.icon||"📄"),1),e("span",ct,_(l.name),1),e("span",mt,_(l.path),1),e("span",ut,"["+_(l.sort_order)+"]",1),N(l.id)>0?(r(),o("span",ht," API: "+_(N(l.id)),1)):E("",!0)]),e("div",gt,[e("button",{onClick:y=>te(l),class:"btn-icon",title:"API 엔드포인트"}," 📡 ",8,vt),e("button",{onClick:y=>ee(l),class:"btn-icon",title:"권한 설정"}," 🔐 ",8,yt),e("button",{onClick:y=>B(l),class:"btn-icon",title:"수정"},"✏️",8,It),e("button",{onClick:y=>de(l),class:"btn-icon",title:"하위 메뉴 추가"}," ➕ ",8,Et),(ue=l.children)!=null&&ue.length?E("",!0):(r(),o("button",{key:0,onClick:y=>Z(l),class:"btn-icon",title:"삭제"}," 🗑️ ",8,Tt))])]),l.children&&l.children.length&&A.value.has(l.id)?(r(),o("div",kt,[(r(!0),o(b,null,M(l.children,y=>(r(),o("div",{key:y.id,class:"tree-node grandchild-node"},[e("div",At,[e("div",Dt,[t[13]||(t[13]=e("span",{class:"expand-icon",style:{visibility:"hidden"}},"▶",-1)),e("span",ft,_(y.icon||"📄"),1),e("span",bt,_(y.name),1),e("span",Mt,_(y.path),1),e("span",Gt,"["+_(y.sort_order)+"]",1),N(y.id)>0?(r(),o("span",Pt," API: "+_(N(y.id)),1)):E("",!0)]),e("div",wt,[e("button",{onClick:se=>te(y),class:"btn-icon",title:"API 엔드포인트"}," 📡 ",8,Nt),e("button",{onClick:se=>ee(y),class:"btn-icon",title:"권한 설정"}," 🔐 ",8,Ct),e("button",{onClick:se=>B(y),class:"btn-icon",title:"수정"}," ✏️ ",8,St),e("button",{onClick:se=>Z(y),class:"btn-icon",title:"삭제"}," 🗑️ ",8,Rt)])])]))),128))])):E("",!0)])}),128))])):E("",!0)])}),128))])])),!S.value&&pe.value.length===0?(r(),o("div",Lt,t[15]||(t[15]=[e("p",null,"메뉴가 없습니다.",-1)]))):E("",!0),O.value?(r(),o("div",{key:3,class:"modal-overlay",onClick:H(j,["self"])},[e("div",Ut,[e("div",Vt,[e("h2",null,_(i.value.id?"메뉴 수정":"새 메뉴 추가"),1),e("button",{onClick:j,class:"close-btn"},"×")]),e("div",Ft,[e("form",{onSubmit:H(Te,["prevent"])},[e("div",Ht,[t[16]||(t[16]=e("label",null,"메뉴명 *",-1)),f(e("input",{"onUpdate:modelValue":t[2]||(t[2]=a=>i.value.name=a),type:"text",class:"form-control",required:"",placeholder:"예: 대시보드"},null,512),[[C,i.value.name]])]),e("div",Wt,[t[17]||(t[17]=e("label",null,"경로 *",-1)),f(e("input",{"onUpdate:modelValue":t[3]||(t[3]=a=>i.value.path=a),type:"text",class:"form-control",required:"",placeholder:"예: /dashboard"},null,512),[[C,i.value.path]])]),e("div",$t,[t[18]||(t[18]=e("label",null,"아이콘",-1)),f(e("input",{"onUpdate:modelValue":t[4]||(t[4]=a=>i.value.icon=a),type:"text",class:"form-control",placeholder:"예: 📊"},null,512),[[C,i.value.icon]])]),e("div",xt,[t[20]||(t[20]=e("label",null,"부모 메뉴",-1)),f(e("select",{"onUpdate:modelValue":t[5]||(t[5]=a=>i.value.parent_id=a),class:"form-control"},[t[19]||(t[19]=e("option",{value:null},"없음 (최상위 메뉴)",-1)),(r(!0),o(b,null,M(Ee.value,a=>(r(),o("option",{key:a.id,value:a.id},_(a.name)+" "+_(a.parent_name?`(${a.parent_name})`:""),9,Bt))),128))],512),[[ne,i.value.parent_id]])]),e("div",jt,[t[21]||(t[21]=e("label",null,"정렬 순서",-1)),f(e("input",{"onUpdate:modelValue":t[6]||(t[6]=a=>i.value.sort_order=a),type:"number",class:"form-control",placeholder:"999"},null,512),[[C,i.value.sort_order,void 0,{number:!0}]])]),e("div",Jt,[e("button",{type:"submit",class:"btn btn-primary",disabled:k.value},_(k.value?"저장 중...":"저장"),9,Qt),e("button",{type:"button",onClick:j,class:"btn btn-secondary"},"취소")])],32)])])])):E("",!0),u.value?(r(),o("div",{key:4,class:"modal-overlay",onClick:H(J,["self"])},[e("div",Kt,[e("div",zt,[e("h2",null,"권한 설정: "+_((p=n.value)==null?void 0:p.name),1),e("button",{onClick:J,class:"close-btn"},"×")]),e("div",qt,[e("div",Xt,[e("table",Yt,[t[22]||(t[22]=e("thead",null,[e("tr",null,[e("th",null,"역할"),e("th",null,"조회 (GET)"),e("th",null,"생성 (POST)"),e("th",null,"수정 (PUT/PATCH)"),e("th",null,"삭제 (DELETE)"),e("th",null,"전체")])],-1)),e("tbody",null,[(r(),o(b,null,M(re,a=>e("tr",{key:a},[e("td",Zt,_(De(a)),1),e("td",ea,[e("input",{type:"checkbox",checked:Q(a,"can_read"),onChange:v=>K(a,"can_read")},null,40,ta)]),e("td",aa,[e("input",{type:"checkbox",checked:Q(a,"can_create"),onChange:v=>K(a,"can_create")},null,40,sa)]),e("td",ia,[e("input",{type:"checkbox",checked:Q(a,"can_update"),onChange:v=>K(a,"can_update")},null,40,na)]),e("td",oa,[e("input",{type:"checkbox",checked:Q(a,"can_delete"),onChange:v=>K(a,"can_delete")},null,40,ra)]),e("td",pa,[e("input",{type:"checkbox",checked:ce(a),onChange:v=>ke(a)},null,40,_a)])])),64))])])]),e("div",da,[e("button",{onClick:Ae,class:"btn btn-primary",disabled:k.value},_(k.value?"저장 중...":"권한 저장"),9,la),e("button",{onClick:J,class:"btn btn-secondary"},"닫기")])])])])):E("",!0),w.value?(r(),o("div",{key:5,class:"modal-overlay",onClick:H(ae,["self"])},[e("div",ca,[e("div",ma,[e("h2",null,"API 엔드포인트: "+_((g=n.value)==null?void 0:g.name),1),e("button",{onClick:ae,class:"close-btn"},"×")]),e("div",ua,[e("div",ha,[t[28]||(t[28]=e("h3",null,"새 엔드포인트 추가",-1)),e("div",ga,[e("div",va,[t[23]||(t[23]=e("label",null,"검색",-1)),f(e("input",{"onUpdate:modelValue":t[7]||(t[7]=a=>L.value=a),type:"text",class:"form-control",placeholder:"경로, 설명, 태그 검색...",onInput:z},null,544),[[C,L.value]])]),e("div",ya,[t[25]||(t[25]=e("label",null,"메소드",-1)),f(e("select",{"onUpdate:modelValue":t[8]||(t[8]=a=>U.value=a),class:"form-control",onChange:z},[t[24]||(t[24]=e("option",{value:""},"전체",-1)),(r(),o(b,null,M(ye,a=>e("option",{key:a,value:a},_(a),9,Ia)),64))],544),[[ne,U.value]])]),e("div",Ea,[t[27]||(t[27]=e("label",null,"태그",-1)),f(e("select",{"onUpdate:modelValue":t[9]||(t[9]=a=>V.value=a),class:"form-control",onChange:z},[t[26]||(t[26]=e("option",{value:""},"전체",-1)),(r(!0),o(b,null,M(Ie.value,a=>(r(),o("option",{key:a,value:a},_(a),9,Ta))),128))],544),[[ne,V.value]])])]),e("div",ka,[e("div",Aa," 사용 가능한 엔드포인트: "+_($.value.length)+"개 ",1),e("div",Da,[(r(!0),o(b,null,M($.value,a=>(r(),o("div",{key:`${a.method}-${a.path}`,class:Se(["available-endpoint-item",{selected:D.value===a}]),onClick:v=>be(a)},[e("span",{class:"method-badge",style:ge({background:me(a.method)})},_(a.method),5),e("div",ba,[e("div",Ma,_(a.path),1),e("div",Ga,_(a.summary),1)]),D.value===a?(r(),o("button",{key:0,type:"button",onClick:H(Me,["stop"]),class:"btn btn-sm btn-primary",disabled:Y.value}," 추가 ",8,Pa)):E("",!0)],10,fa))),128))])])]),e("div",wa,[e("h3",null,"등록된 엔드포인트 ("+_(P.value.length)+"개)",1),P.value.length===0?(r(),o("div",Na," 등록된 API 엔드포인트가 없습니다. ")):(r(),o("div",Ca,[(r(!0),o(b,null,M(P.value,a=>(r(),o("div",{key:a.id,class:"endpoint-item"},[e("div",Sa,[e("span",{class:"method-badge",style:ge({background:me(a.method)})},_(a.method),5),e("span",Ra,_(a.path),1),e("span",Oa,_(a.description),1)]),e("button",{onClick:v=>Ge(a.id),class:"btn-icon",title:"삭제"}," 🗑️ ",8,La)]))),128))]))]),e("div",{class:"form-actions"},[e("button",{onClick:ae,class:"btn btn-secondary"},"닫기")])])])])):E("",!0)])}}}),$a=Re(Ua,[["__scopeId","data-v-be351f5b"]]);export{$a as default};
